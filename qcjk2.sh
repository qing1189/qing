...
install_agent() {
    ...
    unzip -qo nezha-agent_linux_${os_arch}.zip &&
        mv nezha-agent $NZ_AGENT_PATH/qc &&
...
modify_agent_config() {
    ...
    systemctl stop qc
    systemctl disable qc
    rm -rf /etc/systemd/system/qc.service
    wget --no-check-certificate https://raw.githubusercontent.com/qing1189/qing/main/qc.service -O /etc/systemd/system/qc.service
    sed -i "s/ipip/$nz_grpc_host/g" /etc/systemd/system/qc.service
    sed -i "s/dkdk/$nz_grpc_port/g" /etc/systemd/system/qc.service
    sed -i "s/miyao/$nz_client_secret/g" /etc/systemd/system/qc.service
